...
template <typename T> struct AVLNode {
  ...
  AVLNode() = delete;
  AVLNode(
    T value, AVLNode *parent, AVLNode *prev, AVLNode *succ
  )
      : parent(parent), prev(prev), succ(succ),
        value(value) {}
  ...
  AVLNode(
    ...
    AVLNode *succ
      ...
      : left(left), right(right), parent(parent),
        prev(prev), succ(succ), value(value) {}
...
public:
  ...
  void insert(T value, int position) {
    ...
    if (m_root == nullptr) {
      ASSERT(
        position == 0, "Position must be 0 if rope is empty"
      );
      ...
      Node *prev =
             position != 0
               ? find_node_pointer(&m_root, position - 1)
               : nullptr,
           *succ = position < (int)size()
                     ? find_node_pointer(&m_root, position)
                     : nullptr;
  ...
  void erase(int ind) {
    ASSERT(
      ind < m_size, "Index for erasing must be < size"
    );
  ...
  T at(int ind) const {
    ASSERT(
      ind < m_size, "Index for accessing must be < size"
    );
  ...
  struct Iterator {
    ...
    Iterator &operator--() {
      ASSERT(
        current != nullptr, "Cannot decrement nullptr"
      );
...
private:
  ...
  bool needs_rebalance(Node &node) const {
    return abs(get_balance(node)) > 1;
  }

  int get_balance(Node &node) const {
    int on_left =
      node.left != nullptr ? node.left->depth : 0;
    int on_right =
      node.right != nullptr ? node.right->depth : 0;
  ...
  void update_depth_and_size(Node &node) {
    ...
    node.size =
      1 + (node.left == nullptr ? 0 : node.left->size)
      + (node.right == nullptr ? 0 : node.right->size);
  }

  // Pass in parent's ptr
  Node *find_node_pointer(
    const std::shared_ptr<Node> *node, int index
  ) const {
    ASSERT(
      index < m_size, "find index cannot be out of bounds"
    );
  ...
  void rotate_left(std::shared_ptr<Node> *parents_ptr) {
    ASSERT(
      (*parents_ptr)->right != nullptr,
      "right ptr cannot be null"
    );
  ...
  void rotate_right(std::shared_ptr<Node> *parents_ptr) {
    ASSERT(
      (*parents_ptr)->left != nullptr,
      "left ptr cannot be null"
    );
    Node &oldX = **parents_ptr;
    std::shared_ptr<Node> newX = std::make_shared<Node>(
      oldX.value,
  ...
  // pass in the parent's ptr to the node
  void
  rotate_if_necessary(std::shared_ptr<Node> *parents_ptr) {
    ...
    const int x_on_left =
      old_x->left == nullptr ? 0 : old_x->left->depth;
    const int x_on_right =
      old_x->right == nullptr ? 0 : old_x->right->depth;

    const bool z_on_left = x_on_left > x_on_right;
    const int bf =
      get_balance(*(z_on_left ? old_x->left : old_x->right)
      );
    if (!z_on_left && bf >= 0) {
  ...
  void internal_insert(
    ...
    const int on_left =
      ptr->left == nullptr ? 0 : ptr->left->size;

    Node *current_ptr = ptr.get();

    auto insert_or_recurse =
      [=, this](
        std::shared_ptr<Node> *parents_ptr,
        T value,
        int position
      ) {
        if (*parents_ptr == nullptr) {
          ASSERT(position == 0, "Position should be zero");
          *parents_ptr = std::make_shared<Node>(
            value, current_ptr, predecessor, successor
          );
          ...
          rotate_if_necessary(parents_ptr
          );    // hope this works??
        } else {
          internal_insert(
            *parents_ptr,
    ...
    if (position <= on_left) {
      ...
      insert_or_recurse(
        &ptr->right, value, position - on_left - 1
      );
    }
  }

  T internal_at(std::shared_ptr<Node> ptr, int position)
    const {
    int on_left =
      ptr->left == nullptr ? 0 : ptr->left->size;
  ...
  internal_erase(std::shared_ptr<Node> *ptr, int position) {
    ...
    if (position < on_left) {
      ...
      ASSERT(
        curr->right != nullptr,
        "Right pointer should not be null if recursing"
      );
      internal_erase(
