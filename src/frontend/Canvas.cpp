...
void Canvas::set_ortho_projection(float width, float height) {
  ...
  glUniformMatrix4fv(
    glGetUniformLocation(m_geometry_program.get_program(), "projection"),
    1,
    GL_FALSE,
    glm::value_ptr(projection)
  );
...
void Canvas::draw_rectangle(float x, float y, float width, float height) {
  float vertices[12] = {
    x,
    y,
    x + width,
...
std::pair<int, int> Canvas::get_text_dimensions(const std::string &text) const {
